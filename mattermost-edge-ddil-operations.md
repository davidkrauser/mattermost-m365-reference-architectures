...
Network containing enterprise services and continuous access to Microsoft 365. The network may have a firewall or access gateway protecting egress and ingress, such as network policies, IP allowlists, or WAFs depending on your networking configurations.

## 5.2 Enterprise Users

Users within the enterprise network boundary accessing client applications for M365 services and Mattermost.

## 5.3 Tactical Network

Operates independently during disconnections, hosting a self-contained Mattermost deployment to ensure mission users maintain collaboration capabilities.

## 5.4 Mission Users

Tactical or edge users who may be intermittently disconnected from enterprise systems and Microsoft services.
...
### 5.5.1 User Authentication in DDIL Environments
...
### 5.6.1 ID-only Push Notifications
...
## 5.7 Mattermost Server

Core application server handling tactical collaboration workloads when deployed on local infrastructure, such as Azure Local, supporting data residency regulations and disconnected operations. Capabilities include:
...
### 5.7.1 High Availability and Fault Tolerance
...
### 5.7.2 Compliance and Retention
...
## 5.11 Recording Instance

`calls-offloader` job service to offload heavy processing tasks from Mattermost Calls to self-hosted infrastructure within your tactical network, such as recordings, transcriptions, and live captioning. (Optional)

## 5.12 Self-hosted Integrations

Custom apps, plugins, and webhooks can be deployed within your tactical network. (Optional)

## 5.13 Self-hosted LLM

Locally hosted OpenAI compatible LLM for agentic powered collaboration within your tactical network. (Optional)

### 5.13.1 Self-hosted LLMs
...
## 5.14 Microsoft Global Network

World-wide network of Microsoft data centers, delivering public cloud services when internet connectivity permits.

## 5.15 Calls Plugin

The [Calls plugin](https://github.com/mattermost/mattermost-plugin-calls) enables real-time voice calling and screen sharing directly within channels.

### 5.15.1 Self-hosted Audio & Screensharing
